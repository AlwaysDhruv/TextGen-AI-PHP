class EnhancedGeminiChat {
            ...
            handleApiKeyInput() {
                ...
                const hasReachedLimit = this.messageCount >= this.maxMessages;
                this.chatInput.disabled = !hasApiKey || hasReachedLimit;
                this.sendButton.disabled = !hasApiKey || !this.chatInput.value.trim() || hasReachedLimit;
            ...
            async sendMessage() {
                ...
                if (this.messageCount >= this.maxMessages) {
                    this.showError(`📝 You've reached the ${this.maxMessages} message limit for this temporary session. Refresh the page to start a new session.`);
                    return;
                }
            ...
            updateMessageCounter() {
                ...
                this.counterValue.textContent = `${remaining}/${this.maxMessages}`;
                
                // Change color based on remaining messages
                if (remaining === 0) {
                    this.counterValue.style.color = '#ff4444';
                } else if (remaining === 1) {
                    this.counterValue.style.color = '#ffaa00';
                } else {
                    this.counterValue.style.color = 'var(--text-primary)';
                }
            }
            ...
            updateInputPlaceholder() {
                const remaining = this.maxMessages - this.messageCount;
                if (remaining === 0) {
                    this.chatInput.placeholder = "Message limit reached. Refresh to start new session.";
                    this.chatInput.disabled = true;
                    this.sendButton.disabled = true;
                } else if (remaining === 1) {
                    this.chatInput.placeholder = `Last message remaining...`;
                } else {
            ...
            setGeneratingState(isGenerating) {
                ...
                const hasReachedLimit = this.messageCount >= this.maxMessages;
                this.chatInput.disabled = isGenerating || !this.apiKeyInput.value.trim() || hasReachedLimit;
                this.sendButton.disabled = isGenerating || !this.apiKeyInput.value.trim() || !this.chatInput.value.trim() || hasReachedLimit;
                this.sendButton.style.display = isGenerating ? 'none' : 'flex';
                this.stopButton.style.display = isGenerating ? 'flex' : 'none';
                
                if (!isGenerating && !hasReachedLimit) {
            ...
            updateSendButtonState() {
                ...
                const hasApiKey = this.apiKeyInput.value.trim().length > 0;
                const hasReachedLimit = this.messageCount >= this.maxMessages;
                this.sendButton.disabled = !hasMessage || !hasApiKey || this.isGenerating || hasReachedLimit;
            }
            ...
            clearChat() {
                ...
                this.updateInputPlaceholder();
                this.updateMessageCounter();
                this.chatInput.disabled = !this.apiKeyInput.value.trim();
                this.updateSendButtonState();
            }
